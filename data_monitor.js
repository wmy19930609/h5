window.common_monitor=(function($utils,monitor,window,$) {
	var defaultPar={
		...
		isOpenClick:true,
		isOpenTrack:true
	},baseExtend={},start_time=new Date;
  ...
  function getNoObjectAttr(source){
   ...
   {
       var end = +(new Date),start=start_time.getTime(),trackData=monitor.data.getTrack();
       var data={
        intime:start,
        outtime:end,
        type:'leave'
  ...
  var Cookie = {
      get : function(key) {
        try {
          var doc=document,a, reg = new RegExp("(^| )" + key + "=([^;]*)(;|$)");
            ...
            if(!Cookie.get('__mid')){
                 try{
                     ...
                     if(mid){
                         ...
                         Cookie.set('__mid',mid,{expires:date,path:'/',domain:'.btime.com'});
                     }
                 }
                 catch(e){
